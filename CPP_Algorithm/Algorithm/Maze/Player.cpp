...
void Player::AStar()
{
	...
	// G = 시점점에서 해당 좌표까지 이동하는데 드는 비용 ( 작을 수록 좋음, 경로에 따라 달라짐)
	// H = 목적지에서 얼마나 가까운지 (작을 수록 좋음, 고정)
	
	Pos start = _pos; // 시뮬레이션용 좌표
	Pos dest = _board->GetExitPos();
	...
	// OpenList -> 방문은 안했지만 발견한 상태
	priority_queue<PQNode, vector<PQNode>, greater<PQNode>> pq;

	// 1) 예약(발견) 시스템 구현
	// - 이미 더 좋은 경로를 찾았다면 스킵
	// 2) 뒤늦게 더 좋은 경로가 발견될 수 있음 -> 예외 처리 필수
	// - openList에서 차장서 제거한다거나,
	// - pq에서 pop한 다음 무시하거나

	// 초기값
	{
		int32 g = 0; //시작지점에서는 0
		int32 h = 10 * (abs(dest.y - start.y) + abs(dest.x - start.x)); // 구분용 임의의 식 사용함
	...
	while (pq.empty() == false)
	{
		...
		// 동일한 좌표를 여러 경로로 찾아서,
		// 더 빠른 경로로 인해서 이미 방문(closted)된 경우 스킵
		
		// 구성에 따라 clcosed는 필요 없을수도 있음
		...
		for (int32 dir = 0; dir < DIR_COUNT; dir++)
		{
			...
			// 갈 수 있는 지역은 맞는지 확인
			// TODO : 대각선이 추가되었기 때문에 주변 노드가 막혀있을 경우 못가도록 예외처리
			if (CanGo(nextPos) == false)
				continue;
			// 이미 방문한 곳이면 스킵
			if (closed[nextPos.y][nextPos.x] == true)
				continue;
